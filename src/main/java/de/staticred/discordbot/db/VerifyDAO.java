...
public class VerifyDAO {
    ...
    public void addPlayerAsUnverified(ProxiedPlayer player) throws SQLException {
        ...
        DataBaseConnection con = DataBaseConnection.INSTANCE;
        con.connect();
        con.executeUpdate("INSERT INTO verify(UUID,Name,Rank,Grouppower,Online,Verified,DiscordID) VALUES(?,?,?,?,?,?,?)", player.getUniqueId().toString(), player.getName(), PermissionProvider.getGroupName(player.getUniqueId()),PermissionProvider.getGroupJoinPower(PermissionProvider.getGroupName(player.getUniqueId())), true,false,null);
        con.closeConnection();
    }
